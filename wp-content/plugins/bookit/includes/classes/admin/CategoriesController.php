...
class CategoriesController extends DashboardController {
	...
	public static function validate( $data ) {
		...
		if ( ! $data['name'] || ( $data['name'] && strlen( $data['name'] ) < 1 || strlen( $data['name'] ) > 100 ) ) {
			$errors['category_name'] = __( "Category Name can't be empty and must be between 1 and 100 characters long", 'bookit' );
			wp_send_json_error(
				array(
					'errors'  => $errors,
