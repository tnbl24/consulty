	...
	class ElementPack_Settings_API {
		...
		function do_settings_fields($page, $section) {
			...
			foreach ((array) $wp_settings_fields[$page][$section] as $field) {
				...
				printf('<div class="ep-option-item %1$s %2$s" %3$s>', esc_attr($class), esc_attr($widget_used_status), $data_type);
		...
		function callback_text($args) {
			...
			if ($args['video_url']) {
				$html .= '<a href="' . esc_url( $args['video_url'] ) . '" target="_blank" class="ep-option-video" bdt-tooltip="View ' . esc_html( $args['name'] ) . ' Video Tutorial"><i class="bdt-wi-tutorial" aria-hidden="true"></i></a>';
			}
			$html .= sprintf('<label for="bdt_ep_%1$s[%2$s]">', $args['section'], $args['id']);
			$html .= '<span scope="row" class="ep-option-label">' . esc_html( $args['name'] ) . '</span>';
			$html .= '</label>';
